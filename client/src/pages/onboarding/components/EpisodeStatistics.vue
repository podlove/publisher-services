  ...
  <div>
    <dl class="mt-2 grid grid-cols-1 gap-5 sm:grid-cols-3">
      <div class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow sm:p-6">
        ...
        <dd class="mt-1 text-3xl font-semibold tracking-tight text-gray-900">{{ t('onboarding.steps.import-episodes.statistics.numWaitingEpisode', {count: numOfWaitingEpisode}) }}</dd>
      </div>
      <div class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow sm:p-6">
        <dt class="truncate text-sm font-medium text-gray-500">{{ t('onboarding.steps.import-episodes.statistics.successfull') }}</dt>
        <dd class="mt-1 text-3xl font-semibold tracking-tight text-gray-900">{{ t('onboarding.steps.import-episodes.statistics.numSuccessEpisode', {count: numOfSuccessEpisode}) }}</dd>
      </div>
      <div class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow sm:p-6">
        <dt class="truncate text-sm font-medium text-gray-500">{{ t('onboarding.steps.import-episodes.statistics.faulty') }}</dt>
        <dd class="mt-1 text-3xl font-semibold tracking-tight text-gray-900">{{ t('onboarding.steps.import-episodes.statistics.numFaultyEpisode', {count: numOfFaultyEpisode}) }}</dd>
      </div>
    </dl>
  </div>
</template>
