    ...
    <li v-for="(step, stepIdx) in steps" :key="stepIdx" class="relative md:flex md:flex-1">
      <p v-if="step.status === 'complete'" class="group flex w-full items-center">
        <span class="flex items-center px-6 py-4 text-sm font-medium">
          ...
          <span class="ml-4 text-sm font-medium text-gray-900">{{ t(`onboarding.steps.${step.name}.title`) }}</span>
        </span>
      </p>
      <p
      ...
      >
        ...
        <span class="ml-4 text-sm font-medium text-indigo-600">{{ t(`onboarding.steps.${step.name}.title`)}}</span>
      </p>
